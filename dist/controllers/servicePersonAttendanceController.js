...
exports.servicePersonAttendanceController = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
exports.servicePersonAttendanceController = {
    ...
    async getMyAttendance(req, res) {
        try {
            ...
            const { startDate, endDate, status, activityType, search, page = 1, limit = 20 } = req.query;
    ...
    async getMyAttendanceStats(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ error: 'User not authenticated' });
    ...
    async getMyAttendanceDetail(req, res) {
        try {
            const userId = req.user?.id;
