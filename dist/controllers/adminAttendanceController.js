...
exports.adminAttendanceController = {
    ...
    async getAllAttendance(req, res) {
        try {
            ...
            const { startDate, endDate, zoneId, status, userId: filterUserId, activityType, search, page = 1, limit = 20 } = req.query;
            ...
            if (startDate || endDate) {
                ...
                if (startDate) {
                    whereClause.checkInAt.gte = new Date(startDate);
                }
                if (endDate) {
                    whereClause.checkInAt.lte = new Date(endDate);
                }
            }
            // Status filtering
            ...
            if (filterUserId && filterUserId !== 'all') {
                servicePersonsWhere.id = parseInt(filterUserId);
            }
                                ...
                                select: {
                                    activityLogs: {
                                        where: {
                                            ...
                                            ...(activityType && activityType !== 'all' ? { activityType: activityType } : {}),
                                        },
                                    },
                                },
                            },
    ...
    async getAttendanceStats(req, res) {
        try {
            ...
            const [totalRecords, checkedInCount, checkedOutCount, absentCount, lateCount, earlyCheckoutCount, autoCheckoutCount, avgHours] = await Promise.all([
    ...
    async updateAttendance(req, res) {
        try {
            ...
            const { id } = req.params;
            const { checkInAt, checkOutAt, checkInLatitude, checkInLongitude, checkInAddress, checkOutLatitude, checkOutLongitude, checkOutAddress, status, notes, adminNotes } = req.body;
            const attendance = await prisma.attendance.findUnique({
    ...
    async addActivityLog(req, res) {
        try {
            ...
            const { activityType, title, description, startTime, endTime, location, latitude, longitude, ticketId } = req.body;
            const attendance = await prisma.attendance.findUnique({
