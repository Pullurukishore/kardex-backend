...
exports.activityController = {
    ...
    async updateActivity(req, res) {
        try {
            ...
            // Calculate duration if endTime is provided
            let duration = existingActivity.duration ?? undefined;
            if (validatedData.endTime) {
            ...
            let latitude = validatedData.latitude || (existingActivity.latitude ? Number(existingActivity.latitude) : null);
            let longitude = validatedData.longitude || (existingActivity.longitude ? Number(existingActivity.longitude) : null);
            let locationAddress = existingActivity.location;
            // If location is provided as string but lat/lng are null, parse the location
            if (validatedData.location && (!latitude || !longitude)) {
                const parsed = parseLocation(validatedData.location);
    ...
    async getActivities(req, res) {
        try {
            ...
            const { startDate, endDate, activityType, ticketId, includeStages, page = 1, limit = 20 } = req.query;
    ...
    async createTicketActivity(ticketId, userId, oldStatus, newStatus) {
        try {
    ...
    createActivityStage: async (req, res) => {
        try {
    ...
    updateActivityStage: async (req, res) => {
        try {
    ...
    getActivityStages: async (req, res) => {
        try {
            const { activityId } = req.params;
            const user = req.user;
            // Validate activity exists and belongs to user
    ...
    getActivityStageTemplates: async (req, res) => {
        try {
