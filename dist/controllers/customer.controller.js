...
const listCustomers = async (req, res) => {
    ...
    try {
        ...
        if (serviceZoneId) {
            where.serviceZoneId = parseInt(serviceZoneId, 10);
        }
        ...
        if (userRole === 'ADMIN') {
            ...
            const userWithZones = await db_1.default.user.findUnique({
                where: { id: userId },
                include: {
                    serviceZones: {
            ...
            const zoneIds = userWithZones.serviceZones.map((sz) => sz.serviceZoneId);
...
const getCustomer = async (req, res) => {
    ...
    try {
        ...
        const assetLimit = assetsLimit ? parseInt(assetsLimit, 10) : (includeAssets === 'true' ? 1000 : 5);
        ...
        if (userRole === 'ZONE_USER') {
            ...
            const target = await db_1.default.customer.findUnique({ where: { id }, select: { serviceZoneId: true } });
...
const createCustomer = async (req, res) => {
    try {
        const { companyName, address, industry, timezone, serviceZoneId, isActive, contactName, contactPhone, contactEmail } = req.body;
...
const updateCustomer = async (req, res) => {
    try {
        const { id } = req.params;
        const { companyName, address, industry, timezone, serviceZoneId, isActive, contactName, contactPhone } = req.body;
