...
exports.attendanceController = {
    ...
    async checkIn(req, res) {
        try {
            ...
            if (latitude && longitude) {
                try {
                    const { address: geocodedAddress } = await geocoding_service_1.GeocodingService.reverseGeocode(latitude, longitude);
    ...
    async getAllAttendance(req, res) {
        try {
            ...
            const { startDate, endDate, zoneId, status, page = 1, limit = 20 } = req.query;
            const skip = (Number(page) - 1) * Number(limit);
            const whereClause = {};
            // Date filtering
            if (startDate || endDate) {
