...
exports.canViewCustomers = exports.canManageContacts = exports.canManageCustomers = void 0;
...
exports.canManageCustomers = canManageCustomers;
// Middleware to check if user can manage contacts
const canManageContacts = (req, res, next) => {
    try {
        const user = getUserFromRequest(req);
        if (!user) {
            return res.status(401).json({
...
exports.canManageContacts = canManageContacts;
// Middleware to check if user can view customers
const canViewCustomers = (req, res, next) => {
    try {
        const user = getUserFromRequest(req);
        if (!user) {
            return res.status(401).json({
